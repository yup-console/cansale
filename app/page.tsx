...
const SkillIcons: Record<string, React.ReactNode> = {
  Python: (
    <svg width="15" height="15" viewBox="0 0 24 24">
      <path fill="#3572A5" d="M11.914.002C5.82.002 6.2 2.656 6.2 2.656l.007 2.752h5.814v.826H3.888S0 5.77 0 11.927c0 6.157 3.405 5.94 3.405 5.94h2.033v-2.86s-.11-3.404 3.35-3.404h5.77s3.241.052 3.241-3.132V3.296S18.326.002 11.914.002zm-3.2 1.851a1.01 1.01 0 1 1 0 2.02 1.01 1.01 0 0 1 0-2.02z" />
      <path fill="#3572A5" d="M12.086 23.998c6.093 0 5.714-2.654 5.714-2.654l-.007-2.752H11.98v-.826h8.132S24 18.23 24 12.073c0-6.157-3.405-5.94-3.405-5.94h-2.033v2.86s.11 3.404-3.35 3.404h-5.77S6.2 12.345 6.2 15.529v5.175s-.461 3.294 5.886 3.294zm3.2-1.851a1.01 1.01 0 1 1 0-2.02 1.01 1.01 0 0 1 0 2.02z" />
    </svg>
  ),
  Java: (
...
const PROJECTS = [
  { name: "Fault", description: "A Discord bot built with Python — utility & moderation focused", tags: ["Python", "discord.py"], link: "#" },
  { name: "Cellestra", description: "A feature-rich Discord bot crafted in Node.js", tags: ["Node.js", "discord.js"], link: "#" },
  { name: "Crystal Music", description: "High-quality Discord music bot built on Node.js", tags: ["Node.js", "discord.js"], link: "#" },
  { name: "Portfolio", description: "Personal portfolio website with modern design", tags: ["Next.js", "React", "TypeScript"], link: "#" },
];
  ...
  const stars = useMemo(() =>
    Array.from({ length: 140 }, (_, i) => {
      ...
      return {
        ...
        opFrom: group === "c" ? (Math.random() * 0.07 + 0.02).toFixed(2) : (Math.random() * 0.25 + 0.08).toFixed(2),
        opTo: group === "c" ? (Math.random() * 0.9 + 0.55).toFixed(2) : (Math.random() * 0.5 + 0.3).toFixed(2),
    ...
    <>
      ...
      <div className={styles.starsBg}>
        {stars.map((s) => (
          <div
            key={s.id}
            className={`${styles.star} ${styles[`star${s.group.toUpperCase()}`]}`}
            style={{
              left: `${s.x}%`,
              top: `${s.y}%`,
              width: `${s.size}px`,
      ...
      <section ref={homeRef} id="home" className={styles.section}>
        <div className={styles.wrap}>
          <div className={styles.homeName}>Console</div>
          <div className={styles.homeSub}>full stack developer · modern web &amp; discord bots</div>

          <div className={styles.dcCard}>
            <div className={styles.avWrap}>
              {avatarUrl
                ? <img src={avatarUrl} alt="avatar" className={styles.av} />
                : (
          ...
          {spotify && (
            <div className={styles.spCard}>
              <img src={spotify.album_art_url} alt="album" className={styles.spArt} />
              <div style={{ minWidth: 0 }}>
                <div className={styles.spBadge}>Spotify</div>
                <div className={styles.spSong}>{spotify.song}</div>
                <div className={styles.spArtist}>{spotify.artist}</div>
              </div>
            </div>
          )}
        </div>
      ...
      <section ref={projectsRef} id="projects" className={styles.section}>
        <div className={styles.wrap}>
          <div className={styles.secLabel}>a collection of things i&apos;ve worked on</div>
          {PROJECTS.map((p) => (
            <div className={styles.proj} key={p.name}>
              <div style={{ flex: 1 }}>
                <div className={styles.projName}>{p.name}</div>
                <div className={styles.projDesc}>{p.description}</div>
                <div className={styles.projTags}>
                  {p.tags.map((t) => <span className={styles.projTag} key={t}>{t}</span>)}
                </div>
              </div>
      ...
      {/* CONTACT */}
      <section ref={contactRef} id="contact" className={styles.section}>
        <div className={styles.wrap}>
          <div className={styles.secLabel}>feel free to reach out</div>

          <a href="https://discord.com/users/901487880067776524" className={styles.ctItem} target="_blank" rel="noreferrer">
            <div className={styles.ctIco}><IconDiscord /></div>
            <div><div className={styles.ctLabel}>Discord</div><div className={styles.ctVal}>iworship.ayush</div></div>
          </a>
          <a href="https://instagram.com/unkwn.fy" className={styles.ctItem} target="_blank" rel="noreferrer">
            <div className={styles.ctIco}><IconInstagram /></div>
            <div><div className={styles.ctLabel}>Instagram</div><div className={styles.ctVal}>unkwn.fy</div></div>
          </a>
          <a href="https://github.com/yup-console" className={styles.ctItem} target="_blank" rel="noreferrer">
            <div className={styles.ctIco}><IconGithub /></div>
            <div><div className={styles.ctLabel}>GitHub</div><div className={styles.ctVal}>yup-console</div></div>
          </a>
          <a href="mailto:consolepvt@gmail.com" className={styles.ctItem}>
            <div className={styles.ctIco}><IconMail /></div>
            <div><div className={styles.ctLabel}>Email</div><div className={styles.ctVal}>consolepvt@gmail.com</div></div>
          </a>
        </div>
      </section>
    </>
